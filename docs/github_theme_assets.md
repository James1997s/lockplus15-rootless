...
A theme may declare a bounded `assets` array at its root. Each entry needs a safe `id`, a repository-relative image `url`, and the asset file's SHA-256 digest. The downloader accepts only `.jpg`, `.jpeg`, or `.png` image paths, rejects absolute paths and traversal sequences, checks that the URL resolves under the same trusted GitHub host as the catalog, verifies the image data and SHA-256 digest, and stores it under the selected theme's cache directory.
